...
| Pocket ID | Ready in repo | Pinned v2 deployment, Vault-provided encryption key, disabled version/analytics calls, egress restricted to cluster DNS and Alloy, native OTLP, Butler-managed groups, users and OIDC clients, secret rotation into Vault, and Butler PKCE login exist; first owner/API key remain an interactive Titan checkpoint. |
