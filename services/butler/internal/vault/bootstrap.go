...
const (
	...
	pkiRootCommonName    = "Homelab Root CA"
	pkiIntCommonName     = "Homelab Intermediate CA"
	pkiOrganization      = "homelab"
	pkiRootTTL           = "87600h" // 10 years
	pkiIntTTL            = "26280h" // 3 years
	pkiAllowedDomain     = "shivlab.com"
	pkiRoleMaxTTL        = "720h" // 30 days
...
func (c *Client) Bootstrap(ctx context.Context, in BootstrapInput) error {
	...
	if err := c.ensurePKI(ctx); err != nil {
		return err
	}
...
// list on login.)
func firstDomain(_ BootstrapInput) string {
	return "shivlab.com"
}
...
func (c *Client) BootstrapK8sEngine(ctx context.Context, k8s kubernetes.Interface, cfg K8sEngineConfig) error {
	...
	for _, role := range cfg.Roles {
		...
		if _, err := c.raw.Logical().WriteWithContext(ctx, "kubernetes/roles/"+role.Name, map[string]interface{}{
			...
			"kubernetes_namespace":          cfg.HostNamespace,
...
func (c *Client) ensurePKI(ctx context.Context) error {
	mounts, err := c.raw.Sys().ListMountsWithContext(ctx)
	if err != nil {
		return fmt.Errorf("listing mounts: %w", err)
	}
	...
	if !rootExists {
		_, err := c.raw.Logical().WriteWithContext(ctx, pkiRootMount+"/root/generate/internal", map[string]interface{}{
			"common_name":  pkiRootCommonName,
			"organization": pkiOrganization,
			"ttl":          pkiRootTTL,
		})
		if err != nil {
			return fmt.Errorf("generating root CA: %w", err)
		}
		slog.Info("generated root CA", "cn", pkiRootCommonName)
	} else {
	...
	if !intExists {
		...
		slog.Info("generated and signed intermediate CA", "cn", pkiIntCommonName)
	} else {
	...
	if _, err := c.raw.Logical().WriteWithContext(ctx, pkiIntMount+"/roles/"+pkiDefaultRole, map[string]interface{}{
		"allowed_domains":  []string{pkiAllowedDomain},
		"allow_subdomains": true,
		"max_ttl":          pkiRoleMaxTTL,
		"key_type":         "rsa",
		"key_bits":         2048,
	}); err != nil {
